...
int main(int argc, char *argv[])
{
    ...
    clock_t decr_begin, decr_end;
    double decr_time_spent;
	...
	message_to_send[3] = 'A';
	message_to_send[4] = '\0';*/
    memcpy(message_to_send, argv[1], strlen(argv[1]));
    message_to_send[strlen(argv[1])] = '\0';

    std::vector<std::pair <unsigned char*, unsigned int> > p;
    p = Base64Splitter(message_to_send, strlen(argv[1]), 32);
    ...
    for(unsigned int j=0; j<p.size(); j++)
    {
        ...
        decr_end = clock();
        decr_time_spent = (double)(decr_end - decr_begin) / CLOCKS_PER_SEC;
